...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (function(){
    ...
    // drag & drop
    if(avatarEl){
      avatarEl.addEventListener('dragover', (ev)=>{ ev.preventDefault(); avatarEl.classList.add('dragover'); });
      avatarEl.addEventListener('dragleave', (ev)=>{ ev.preventDefault(); avatarEl.classList.remove('dragover'); });
      avatarEl.addEventListener('drop', (ev)=>{
        ev.preventDefault(); avatarEl.classList.remove('dragover');
        const f = ev.dataTransfer && ev.dataTransfer.files && ev.dataTransfer.files[0];
        if(!f) return;
        const reader = new FileReader();
        reader.onload = function(e){ const data = e.target.result; try{ localStorage.setItem('profileAvatar', data); }catch(err){} showImage(data); };
        reader.readAsDataURL(f);
      });
    }
    ...
    document.addEventListener('paste', (ev)=>{
      ...
      for(const it of items){
        if(it.type && it.type.indexOf('image') === 0){
          ...
          const reader = new FileReader();
          reader.onload = (e)=>{ const data = e.target.result; try{ localStorage.setItem('profileAvatar', data); }catch(err){} showImage(data); };
          reader.readAsDataURL(f);
          ev.preventDefault();
          return;
        }
    ...
    if(removeBtn){
      removeBtn.addEventListener('click', ()=>{
        try{ localStorage.removeItem('profileAvatar'); } catch(e){}
        clearImage();
        const n = document.getElementById('name-text')?.innerText || 'Châu Thế Phong';
        if(avatarInitials) avatarInitials.innerText = computeInitials(n);
      });
    }
